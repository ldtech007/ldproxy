...
proxy_config_t g_proxycli_cfg = {0};
...
static int on_data_relay(ld_session_t* session, struct epoll_event* session_event, int* cli_fd, int* svr_fd, LISTHEAD* from_cli_data_list, LISTHEAD* from_svr_data_list) {
    ...
    if (session_event->events & EPOLLIN) {
        ...
        if (rdlen < 0) {
            ...
            if (session->event_fd == CLI_FD) {
                log_debug("encode from cli data, session:%p\n", session);
                ciper_encode((unsigned char*)packet->data, packet->len);
            } else {
                log_debug("decode from svr data, session:%p\n", session);
    ...
    for (i = 0; i < epfds; ++i) {
        ...
        if (SVRFDFLAG_ISSET(g_session_events[i].data.ptr)) {
            ...
            if (session->cli_event_status == CLI_EVENT_STATUS_BUILD_SVR) {
                ...
                svr_addr.sin_port = htons((uint16_t)g_proxycli_cfg.server_port); 
                svr_addr.sin_addr.s_addr = inet_addr((const char*)g_proxycli_cfg.server_addr);
...
int init_agent_service() {
    ...
    g_agent_server.fd = -1;
    g_agent_server.port = (uint16_t)g_proxycli_cfg.local_port; 
    g_agent_server.addr = g_proxycli_cfg.local_addr;
