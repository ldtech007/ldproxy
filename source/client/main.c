...
extern proxy_config_t g_proxycli_cfg;
...
int main(int argc, char* argv[]) {
    ...
    timer_ticks_init();
    ret = load_cli_proxy(&g_proxycli_cfg);
    if (ret < 0) {
        log_fault("load_cli_proxy failed\n");
        goto error;
    } 
    ret = ciper_init(g_proxycli_cfg.secretkey, SECRET_KEY_LEN);
    if (ret < 0) {
        log_fault("ciper_init failed\n");
        goto error;
    }
    ret = client_session_pool_init(g_max_session);
    if (ret < 0) {
        log_fault("client_session_pool_init failed\n");
        goto error;
    }
...
error:
    ...
    if (g_proxycli_cfg.local_addr) {
        free(g_proxycli_cfg.local_addr);
        g_proxycli_cfg.local_addr = NULL;
    }
    if (g_proxycli_cfg.server_addr) {
        free(g_proxycli_cfg.server_addr);
        g_proxycli_cfg.server_addr = NULL;
    }

    close_agent_service();
    client_session_pool_uninit();
    ldlog_uninit();
    return ret;
}