...
static ld_session_t* __malloc_server_session(ld_session_t* session_pool, unsigned int max_session);
...
static ld_session_t* __malloc_server_session(ld_session_t* session_pool, unsigned int max_session) {
    ...
    session_pool[i].h = h3_request_header_new();
    session_pool[i].update_ticks = g_cur_ms;
    
    return &(session_pool[i]);
}

ld_session_t* malloc_server_session() {

    return __malloc_server_session(g_sessions, g_max_session);
}

void free_server_session(ld_session_t* session) {
    if (session == NULL) {
       return;
    }
    if (session->flag == 0) {
    ...
    if (session->h) {
        h3_request_header_free(session->h);
        session->h = NULL;
    }
...
int server_session_pool_init(unsigned int max_sesssion) {
...
void server_session_pool_uninit()
{
    if (NULL != g_sessions)
    for (int i = 0; i < g_max_session; ++i) { 
        free_server_session(&g_sessions[i]);
    }
    free(g_sessions);
    g_sessions = NULL;
}
