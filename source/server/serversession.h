...
enum {
    ...
    CLI_EVENT_STATUS_PARSE_HEAD,
    CLI_EVENT_STATUS_PROCESS_HEAD,
...
typedef struct
{
    ...
    RequestHeader *h;
    int req_head_cache_len; //已缓存的头部长度
    unsigned char req_head_cache[LD_BUFFER_SIZE_8k + 1]; //缓存请求头部，用来解析头部字段
} ld_session_t;
